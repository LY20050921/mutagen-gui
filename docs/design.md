...
| F11.9 | **路径脱敏展示**：所有**只读展示**的路径经 `config.display_path()` 折叠 —— 当前用户主目录下折叠成 `~/...`，其它盘符下的 `C:\Users\<谁>\` 折叠成 `C:\Users\<user>\`，其余原样。目的：用户**截图分享**界面时不泄露 Windows 用户名。⚠️ **可编辑输入框与「复制路径」保持真实值**（否则保存会失效）。已落成 `--check` 断言（`_check_path_masking`）|
